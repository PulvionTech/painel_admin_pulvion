...
CREATE INDEX IF NOT EXISTS idx_aplicacao_produtos_aplicacao
  ON aplicacao_produtos(aplicacao_id);

CREATE INDEX IF NOT EXISTS idx_aplicacao_produtos_enterprise
  ON aplicacao_produtos(enterprise_id);
...
SELECT
  enterprise_id,
  id,
  COALESCE(NULLIF(classe_produto, ''), 'Não informado'),
  produto_nome,
  COALESCE(NULLIF(dosagem, 0), 1),
  COALESCE(NULLIF(unidade, ''), 'un/ha'),
  COALESCE(dosagem, 0) * area_ha,
  NULLIF(num_art, '')
FROM aplicacoes
WHERE produto_nome IS NOT NULL
  AND produto_nome <> ''
  AND NOT EXISTS (
    SELECT 1 FROM aplicacao_produtos produto
    WHERE produto.aplicacao_id = aplicacoes.id
  );

ALTER TABLE aplicacao_produtos ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS admin_insert_aplicacoes ON aplicacoes;
CREATE POLICY admin_insert_aplicacoes ON aplicacoes
  FOR INSERT WITH CHECK (current_role_is_admin());

DROP POLICY IF EXISTS tenant_isolation ON aplicacao_produtos;
CREATE POLICY tenant_isolation ON aplicacao_produtos
  USING (enterprise_id = current_enterprise_id());

DROP POLICY IF EXISTS admin_write_aplicacao_produtos ON aplicacao_produtos;
CREATE POLICY admin_write_aplicacao_produtos ON aplicacao_produtos
...
DROP TRIGGER IF EXISTS trg_aplicacao_produtos_updated_at ON aplicacao_produtos;
CREATE TRIGGER trg_aplicacao_produtos_updated_at
  BEFORE UPDATE ON aplicacao_produtos
...
BEGIN
  ...
  DELETE FROM aplicacao_produtos WHERE aplicacao_id = v_aplicacao_id;
